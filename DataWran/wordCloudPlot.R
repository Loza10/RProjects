...
wordcloud::wordcloud(words=toupper(finalDF$word), freq=finalDF$count, max.words=10)

data(schoolProgram, package='syllogi')
head(schoolProgram)

read <- schoolProgram$read

graphics.off()
h <- hist(read, breaks=40)

h$counts / sum(h$counts)

sum(h$density)

hist(read, xlab='Reading', main='', freq=FALSE, ylim=c(0, 0.06))
plot(density(read))

read
graphics.off()
b <- boxplot(c(150, read))
b
b$stats
summary(read)
